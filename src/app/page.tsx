...
const handleGenerate = async () => {
  ...
  try {
    ...
    if (data?.imageUrl) {
      setProjects(p => [{ id: crypto.randomUUID(), imageUrl: data.imageUrl, prompt, user: user.email }, ...p]);
      setPrompt('');
    } else {
  ...
  return (
    <main className="min-h-screen p-6">
      <header className="mb-6 flex justify-between">
        ...
        <div>
          {user ? (
            ...
            <button onClick={signIn} className="border rounded px-3 py-1">Zaloguj się</button>
          )}
        </div>
      </header>
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {projects.length === 0 && <p className="col-span-full text-gray-500">Na razie pusto – wygeneruj coś!</p>}
        {projects.map((p) => (
